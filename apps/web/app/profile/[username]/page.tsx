...
export default async function PublicProfilePage({ params }: ProfilePageProps) {
  ...
  return (
    <div className="min-h-screen pb-20 animate-in fade-in duration-500">
      <div className="-mx-4 md:-mx-8 -mt-4 md:-mt-8 h-64 md:h-80 relative overflow-hidden border-b border-border/50 shadow-2xl rounded-b-4xl transition-all duration-700 z-0" style={{ background: styles.background || '#18181b' }}>
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-20 mix-blend-overlay"></div>
        <div className="absolute inset-0 bg-linear-to-t from-background via-background/40 to-transparent" />
      </div>

      <div className="max-w-6xl mx-auto -mt-24 md:-mt-32 relative z-10 px-4 md:px-0">
        <div className="bg-surface/60 backdrop-blur-xl border border-border/60 rounded-3xl p-6 md:p-8 shadow-2xl flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start">
          ...
          <div className="flex-1 text-center md:text-left space-y-3 w-full">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight flex items-center justify-center md:justify-start gap-3 drop-shadow-md">
                  {profile.username}
                  <span className="text-xs font-black bg-primary/20 text-primary border border-primary/50 px-2.5 py-1 rounded-lg uppercase tracking-wider">Lvl {profile.global_level || 1}</span>
                </h1>
                <div className="mt-2">
                  {styles.titleStyle && styles.titleName ? (
                    <span className="inline-block px-4 py-1.5 rounded-lg border border-white/20 text-xs font-black shadow-lg text-white" style={{ background: styles.titleStyle }}>{styles.titleName}</span>
                  ) : (
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-2 mt-4">
                  {badges.map((ub) => {
                    ...
                    return (
                      <div key={ub.badge_id} title={`${ub.name}: ${ub.description}`} className={`flex items-center gap-1.5 px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg cursor-help transition-all hover:bg-white/10 relative ${ub.color_class} ${isNew ? 'ring-2 ring-primary animate-pulse' : ''}`}>
            ...
            <p className="text-gray-300 leading-relaxed text-sm max-w-2xl mx-auto md:mx-0 bg-background/40 p-4 rounded-xl border border-white/5">&quot;{profile.bio || "Este caçador prefere manter o mistério."}&quot;</p>
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-2">
              <div className="flex items-center gap-4 bg-background/60 px-4 py-2 rounded-xl border border-border/50">
                <Link href={`/profile/${profile.username}/network?tab=followers`} className="text-xs text-gray-400 font-bold uppercase tracking-wider hover:text-primary transition-colors"><strong className="text-white text-base mr-1">{followersCount || 0}</strong> Seguidores</Link>
                <div className="w-px h-4 bg-border"></div>
                <Link href={`/profile/${profile.username}/network?tab=following`} className="text-xs text-gray-400 font-bold uppercase tracking-wider hover:text-primary transition-colors"><strong className="text-white text-base mr-1">{followingCount || 0}</strong> Seguindo</Link>
              </div>
              <span className="text-[11px] text-gray-500 font-bold uppercase tracking-widest px-3 py-2 bg-surface rounded-xl border border-border/50">📅 Entrou em {joinDate}</span>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-0 mt-8 grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between border-b border-border/50 pb-3">
            <h2 className="text-2xl font-black text-white flex items-center gap-3">🏆 Estante de Troféus</h2>
            <span className="text-xs font-bold text-gray-400 bg-surface px-3 py-1.5 rounded-lg border border-border">{showcaseGames.length} / {showcaseLimit} Slots</span>
          </div>
          {showcaseGames.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              ...
              {Array.from({ length: Math.max(0, showcaseLimit - showcaseGames.length) }).map((_, i) => (
                <div key={`empty-${i}`} className="aspect-3/4 rounded-2xl border-2 border-dashed border-border/30 bg-surface/20 flex flex-col items-center justify-center gap-2 opacity-50"><span className="text-2xl">🔒</span><span className="text-[9px] font-black uppercase tracking-widest text-gray-500">Slot Disponível</span></div>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="space-y-6 lg:sticky lg:top-24">
          <div className="flex items-center justify-between border-b border-border/50 pb-3">
            <h2 className="text-2xl font-black text-white flex items-center gap-3">💬 Mural</h2>
          </div>
          <div className="bg-surface/50 backdrop-blur-sm border border-border rounded-3xl p-4 flex flex-col shadow-xl relative overflow-visible h-auto max-h-175">
            {authUser ?
              <div className="mb-4 relative z-10">
                <CommentInput profileId={profile.id} currentPath={currentPath} />
              </div> :
              <div className="mb-4 p-4 bg-background/80 rounded-2xl border border-border text-center text-sm text-gray-400 font-bold shadow-inner">Inicie sessão para deixar um recado.</div>}
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar relative z-10 overflow-visible">
