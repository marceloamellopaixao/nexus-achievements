    ...
    if (match) {
      return (
        <div key={index} className="relative w-full aspect-video my-6">
          <Image src={match[2] as string} alt={match[1] || 'Imagem do guia'} fill className="rounded-2xl border border-border shadow-2xl object-contain" unoptimized />
        </div>
      );
    }
      ...
      <div className="max-w-6xl mx-auto -mt-32 md:-mt-48 relative z-20 px-4 md:px-0">
        <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-start md:items-end">
          ...
          <div className="flex-1 w-full pb-2 md:pb-6">
            <h1 className="text-3xl md:text-6xl font-black text-white tracking-tighter drop-shadow-2xl leading-none">{game.title}</h1>
            <p className="text-primary font-bold tracking-widest uppercase text-xs md:text-sm mt-3 md:mt-4 bg-primary/10 inline-block px-3 py-1 rounded-md border border-primary/20">{game.developer || "Steam"}</p>

            {userProgress && (
              <div className={`mt-6 md:mt-8 p-6 rounded-3xl border backdrop-blur-md relative overflow-hidden transition-all duration-500 shadow-xl ${isPlat ? 'bg-cyan-950/40 border-cyan-500/50 shadow-cyan-900/20' : 'bg-surface/80 border-border'}`}>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-3">
                        <h3 className="text-sm font-black uppercase tracking-widest text-white">
                          {isPlat ? <span className="text-cyan-400">👑 Platina Conquistada</span> : percentage > 0 ? "⚔️ Jornada em curso" : "🎯 Iniciar Caçada"}
                        </h3>
                        <span className={`text-sm md:text-base font-black ${isPlat ? 'text-cyan-400' : 'text-gray-300'}`}>
                  ...
                  <div className="bg-background/60 px-5 py-3 rounded-2xl border border-white/5 flex flex-col items-center justify-center min-w-30 shadow-inner">
                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">Tempo de Jogo</span>
                    <span className="text-xl font-black text-white">{playtimeHours}h {playtimeMins}m</span>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-6">
              <h2 className="text-2xl font-black text-white flex items-center gap-3"><span className="text-3xl">🏆</span> Todas as Conquistas</h2>
                    ...
                    <div key={ach.name} className={`flex items-start gap-4 border p-4 rounded-2xl transition-all duration-300 group ${isUnlocked ? 'border-primary/30 bg-primary/5 shadow-sm' : 'border-border/40 bg-surface/30 opacity-70 grayscale hover:grayscale-0 hover:opacity-100'}`}>
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          {isUnlocked && <span className="text-[10px] font-black text-green-500 bg-green-500/10 px-1.5 rounded border border-green-500/20">OBTIDO</span>}
                        </div>
