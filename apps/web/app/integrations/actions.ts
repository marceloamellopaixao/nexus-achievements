...
export async function processSingleGame(game: SteamGame, steamId: string) {
  ...
  try {
    // 🔥 Agora usa l=brazilian para buscar em português
    ...
    // 🚀 PUXANDO GÊNEROS OFICIAIS DA LOJA DA STEAM EM PORTUGUÊS!
    ...
    // 🔥 SÓ PROCESSA E DÁ MOEDAS SE TIVER CONQUISTAS NOVAS
    if (unlockedCount > previousUnlocked) {
      const schemaMap = new Map<string, { displayName: string, icon: string }>()
      
      // 🔥 Busca os Nomes Oficiais Traduzidos para o Banco
